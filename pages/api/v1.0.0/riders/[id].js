...
export default authenticate(async (req, res) => {
  ...
  try {
    //if riders doesn't get resolved, the id wont be passed, fix the apiKey issue
    ...
    if (match) {
      ...
      if (method === "PUT") {
        ...
        const response = await updateOneEntry("riders", id, set);
        ...
        console.log("method to use", method);
        const response = await deleteOne("riders", id);
        ...
        console.log("method to use", method);
        const response = await findOne("riders", {
          _id: ObjectId(id),
        });
